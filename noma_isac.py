...
for idx, snr_db in enumerate(SNR_dB_outage):
    ...
    for trial in range(num_trials):
        # 基于实时信道反馈的动态功率分配
        alpha_N = alpha_N_fixed + 0.1 * random.random()  # alpha_N ∈ [0.2, 0.3]
        alpha_F = alpha_F_fixed - 0.1 * random.random()  # alpha_F ∈ [0.5, 0.8]
...
for mu_idx, mu in enumerate(mu_range):
    ...
    for trial in range(num_trials):
        # ISAC的动态功率分配
        alpha_N = alpha_N_fixed + 0.1 * random.random()
        alpha_F = alpha_F_fixed - 0.1 * random.random()
...
with open('./log.txt', 'w', encoding='utf-8') as f:
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('outage_probability.png')
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('sensing_rate_vs_snr.png')
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('ergodic_communication_rate.png')
    plt.show()
    ...
    print(f'sum_ECR_FDSAC_mu={sum_ECR_FDSAC_mu.tolist()}\tsensing_rate_FDSAC_mu={sensing_rate_FDSAC_mu.tolist()}', file=f)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('sensing_vs_communication_rate.png')
    plt.show()
